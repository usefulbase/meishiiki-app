...
(function(){
  ...
  function toggleCompact(){
    ...
    appendViewMap({keepScroll:false});
  }
  ...
  function buildSvgForEye(eye){
    ...
    const width = compactMap ? 430 : 760;
    const height = compactMap ? (lensType === "progressive" ? 220 : 245) : (lensType === "progressive" ? 300 : 340);
    const left = compactMap ? 66 : 82;
    const top = compactMap ? 24 : 34;
    const bottom = compactMap ? 36 : 54;
    ...
    positions.forEach((pos, rowIndex) => {
      const y = top + rowIndex * rowH + (compactMap ? 2 : 3);
      DISTANCES.forEach((dist, colIndex) => {
        ...
        if (score <= 0) return;
        const x = left + colIndex * colW + (compactMap ? 2 : 4);
        const h = Math.max(4, rowH - (compactMap ? 4 : 6));
        const w = Math.max(6, colW - (compactMap ? 4 : 8));
        cells += `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="${compactMap ? 5 : 7}" fill="${cellColor(score)}" />`;
      });
    });

    let xLabels = "";
    if (compactMap) {
      xLabels = `<text x="${left + plotW * 0.08}" y="${height - 14}" text-anchor="middle" font-size="11" fill="#475569">遠方</text><text x="${left + plotW * 0.52}" y="${height - 14}" text-anchor="middle" font-size="11" fill="#475569">中間</text><text x="${left + plotW * 0.92}" y="${height - 14}" text-anchor="middle" font-size="11" fill="#475569">手元</text>`;
    } else {
    ...
    model.labels.forEach(label => {
      ...
      yLabels += `<text x="${compactMap ? 8 : 12}" y="${y}" font-size="${compactMap ? 11 : 12}" font-weight="700" fill="${label.color}">${label.text}</text>`;
    });
  ...
  function buildMapHtml(){
    ...
    const maps = eyes.map(eye => {
      ...
      return `<div class="view-map-box"><div class="view-map-title"><strong>${label} 距離別見え方マップ</strong><span>${rowsText} / ${compactMap ? "簡易表示" : "濃いほど調節負担が少ない目安"}</span></div><div class="view-map-svg-wrap">${buildSvgForEye(eye)}</div></div>`;
    }).join("");

    const fpLegend = getTextValue("lensType") === "indoor" ? "<span>破線：FP位置</span>" : "";
    const detailsClass = compactMap ? "view-map-details is-compact" : "view-map-details";
    const openAttr = mapOpen ? " open" : "";
    return `<details class="${detailsClass}"${openAttr}><summary>距離別見え方マップを表示</summary>${maps}<div class="view-map-box view-map-control-box">${controls}</div><div class="view-map-box"><div class="view-map-legend"><span><i class="view-map-dot dark"></i>調節負担が少ない</span><span><i class="view-map-dot light"></i>調節負担が大きい</span><span>空白：範囲外の目安</span>${fpLegend}</div><p class="view-map-note">※このマップは、正確なレンズ設計再現ではなく、加入量を上下方向に補間した説明用モデルです。各距離で必要な調節量を「必要調節量＝距離の逆数D−その位置の完全矯正との差」で求め、必要調節量が0以上かつ使用調節力以内なら表示しています。濃さは調節負担の少なさの目安です。実際の見え方は、レンズ設計、明瞭域の幅、視線、姿勢、フィッティング、慣れなどでも変わります。</p></div></details>`;
  }

  function bindControlEvents(container){
    const details = container.querySelector(".view-map-details");
    if (details) {
      details.addEventListener("toggle", () => {
        mapOpen = details.open;
      });
    }
    container.querySelectorAll("[data-view-map-action]").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        if (action === "toggle-compact") toggleCompact();
      });
    });
  }
  ...
  function getScrollPositions(){
    ...
    if (!existing) return [];
    const details = existing.querySelector(".view-map-details");
    if (details) mapOpen = details.open;
    return Array.from(existing.querySelectorAll(".view-map-svg-wrap")).map(el => el.scrollLeft || 0);
  }

  function restoreScrollPositions(scrolls){
    if (!Array.isArray(scrolls) || compactMap) return;
    requestAnimationFrame(() => {
      const container = document.getElementById("viewMapContainer");
      if (!container) return;
      container.querySelectorAll(".view-map-svg-wrap").forEach((el, i) => {
        if (typeof scrolls[i] === "number") el.scrollLeft = scrolls[i];
      });
    });
  }

  function appendViewMap(options = {}){
    const result = document.getElementById("result");
    if (!result) return;
    const scrolls = options.keepScroll ? getScrollPositions() : [];
    const existing = document.getElementById("viewMapContainer");
